...
pipeline {
    ...
    stages {
        stage('Clone Repo') {
            steps {
                git url: "https://github.com/saylee12patil/EasyCRUD-docker-updated.git", branch: "main"
            }
        }

        stage('Build Frontend Image') {
            steps {
                sh "docker build -t saylee12patil/easycrud1-jenkins:frontend ./frontend"
            }
        }

        stage('Build Backend Image') {
            steps {
                sh "docker build --no-cache -t saylee12patil/easycrud1-jenkins:backend ./backend"
            }
        }

        stage('Docker Hub Login') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'dockerhub-cred', usernameVariable: 'saylee', passwordVariable: 'redhat123')]) {
        ...
        stage('Push Frontend Image to Docker Hub') {
            steps {
                sh "docker push saylee12patil/easycrud1-jenkins:frontend"
            }
        }

        stage('Push Backend Image to Docker Hub') {
            steps {
                sh "docker push saylee12patil/easycrud1-jenkins:backend"
            }
        }
        ...
        stage('Run Frontend Container') {
            steps {
                sh '''
                    ...
                    docker run -d --name easycrud1-frontend -p 80:80 saylee12patil/easycrud1-jenkins:frontend
                '''
            }
        }
        ...
        stage('Run Backend Container') {
            steps {
                sh '''
                    ...
                    docker run -d --name easycrud1-backend -p 8080:8080 saylee12patil/easycrud1-jenkins:backend
                '''
            }
        }
    }
